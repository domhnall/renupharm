class Marketplace::CreditCardsController < AuthenticatedController
  def create
    ...
    if @credit_card.valid? && @credit_card.save && @credit_card.authorize!(shopper_ip: request.remote_ip)
      redirect_to pharmacy_credit_cards_path, flash: { success: t('marketplace.credit_card.flash.create_successful') }
    else
      redirect_to pharmacy_credit_cards_path, flash: { warning: t('marketplace.credit_card.flash.error') }
    end
