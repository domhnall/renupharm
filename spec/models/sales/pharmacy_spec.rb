...
describe Sales::Pharmacy do
  ...
  describe "initialization" do
    ...
    [:telephone, :fax].each do |attr|
      it "should be invalid when :#{attr} exceeds length of 16 characters" do
        expect(Sales::Pharmacy.new(@params.merge("#{attr}" => "+353 12345678901"))).to be_valid
        expect(Sales::Pharmacy.new(@params.merge("#{attr}" => "+353 123456789012"))).not_to be_valid
      end

      it "should be invalid when :#{attr} has length of less than 11 characters" do
        expect(Sales::Pharmacy.new(@params.merge("#{attr}" => "+353 123456"))).to be_valid
        expect(Sales::Pharmacy.new(@params.merge("#{attr}" => "+353 12345"))).not_to be_valid
      end
    end

    [:telephone, :fax].each do |attr|
      describe "setting :#{attr}" do
        { "41-123-4567" => "+353 411234567",
          "(41) 123 4567" => "+353 411234567",
          "(01) 283 7188" => "+353 12837188",
          "1 283 7188" => "+353 12837188" }.each do |supplied, cleaned|
