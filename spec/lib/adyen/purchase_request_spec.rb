...
describe Adyen::PurchaseRequest do
  subject { Adyen::PurchaseRequest.new }

  it 'should define a non-empty array of REQUEST_FIELDS' do
    expect(Adyen::PurchaseRequest::REQUEST_ATTRIBUTES).not_to be_nil
    expect(Adyen::PurchaseRequest::REQUEST_ATTRIBUTES).to be_an Array
    expect(Adyen::PurchaseRequest::REQUEST_ATTRIBUTES).not_to be_empty
  end

  it 'should define a non-empty hash of ADYEN_REQUEST_KEYS' do
    expect(Adyen::PurchaseRequest::ADYEN_REQUEST_KEYS).not_to be_nil
    expect(Adyen::PurchaseRequest::ADYEN_REQUEST_KEYS).to be_a Hash
    expect(Adyen::PurchaseRequest::ADYEN_REQUEST_KEYS).not_to be_empty
  end

  Adyen::PurchaseRequest::REQUEST_ATTRIBUTES.each do |method|
  ...
  [ :build_adyen_params ].each do |method|
    it "should respond to the instance method '#{method}'" do
      expect(subject).to respond_to method
    end
  end
  ...
  describe 'instantiation' do
    let(:params) { {} }
    Adyen::PurchaseRequest::REQUEST_ATTRIBUTES.each do |attr|
      it "should correctly initialize attribute :#{attr} when supplied" do
        expect(Adyen::PurchaseRequest.new(params.merge({attr => 'test'})).send(attr)).to eq 'test'
      end

      unless [ :action, :recurring_contract ].include?(attr)
        it "should set the attribute :#{attr} to nil when not supplied" do
          expect(Adyen::PurchaseRequest.new({}).send(attr)).to be_nil
        end
      end
    end

    it "should set the :action attribute to 'Payment.authorise' if not supplied" do
      expect(Adyen::PurchaseRequest.new({}).action).to eq 'Payment.authorise'
    end

    it "should set the :recurring_contract attribute to 'RECURRING' if not supplied" do
      expect(Adyen::PurchaseRequest.new({}).recurring_contract).to eq 'RECURRING'
    end
  end
  ...
  describe 'instance method' do
    ...
    subject { Adyen::PurchaseRequest.new(params) }

    describe '#build_adyen_params' do
      let(:generated_hash) { subject.build_adyen_params }

      Adyen::PurchaseRequest::ADYEN_REQUEST_KEYS.each do |attr,key|
      ...
      Adyen::PurchaseRequest::ADYEN_REQUEST_KEYS.select{ |k,v| [:shopper_email, :shopper_ip].include?(k)}.each do |attr, key|
    ...
    describe '#valid?' do
      ...
      [ :action,
        ...
        :merchant_account,
        :reference,
        :shopper_reference ].each do |mandatory_attr|
